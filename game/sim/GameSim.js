 ...
 *
 * The ISOMORPHIC SIMULATION CORE (Phase 0 of the server-authoritative move).
 *
 * This module owns everything that decides what happens in a match: players,
 * platformer physics, weapons, skills, projectiles, mines, fire patches, status
 * effects, bots, healing items, kill credit and the match timer.
 *
 * It runs UNCHANGED in two places:
 *   - the browser, for the one-click offline bot match (Game extends it)
 *   - Node, inside the dedicated game server, which owns every online room
 *
 * Therefore it must never touch the DOM, a canvas, the renderer, the camera, an
 * input device, the audio engine, or a network transport. Instead:
 *   - juice goes to an injected presentation sink   -> this.fx      (sim/Fx.js)
 *   - randomness + time are injected                -> this.rng/now (sim/env.js)
 *   - input arrives only via applyInput/applyAim
 *   - outbound messages are QUEUED via _publish, and the runner drains them
 *
 * The sim has no notion of a "local player": cues are tagged with the player
 * they belong to and the sink decides whether that player is you.
 */
...
export class GameSim {
  constructor(options = {}) {
    // Dummy (practice) room: the authority spawns stationary training dummies.
    this.dummyRoom = !!options.dummyRoom;
    this.dummyCount = Number.isFinite(options.dummyCount) ? options.dummyCount : 3;

    // Bot match: fill the arena with AI bots (synthetic-input Players).
    ...
    // Injected ambient dependencies (see sim/env.js): a server supplies its own
    // seed + clock. Sim code uses this.rng()/this.now(), never Math.random()/Date.now().
    this.seed = Number.isFinite(options.seed) ? (options.seed >>> 0) : randomSeed();
    this.rng = makeRng(this.seed);
    this.clock = options.clock || SystemClock;

    // Short-round support: optional time limit / kill target.
    ...
    // The rising-lava hazard was removed; a null zone makes every guard no-op.
    this.zone = null;
  ...
  /** Clear every per-match entity. Called before a match (re)starts. */
  resetEntities() {
    this.players = {};
    this.projectiles = [];
    this.pendingSwordWaves = [];
  ...
  /**
   * Authority-side match setup: build the terrain from the seed, seat the first
   * player, then fill in dummies/bots. Returns the seated Player.
   *
   * The caller supplies the player's identity because the sim has no idea where
   * a nickname or a costume comes from (a DOM field in the browser, a JOIN_ROOM
   * payload on the server).
   */
  beginMatch({ id, nickname, weapon, costume = null, isMobile = false, mobileAimAssist = false, automaticAttack = false }) {
    this.isAuthority = true;
    this.matchStartTime = this.now();
    ...
    player.mobileAimAssist = !!mobileAimAssist;
    player.automaticAttack = !!automaticAttack;
    this._enforceWorkshopPolicy(player);
    this.players[id] = player;

    if (this.dummyRoom) this._spawnDummies(this.dummyCount);
    if (this.botMatch) {
...
// Default presentation sink: headless. The browser shell installs a BrowserFx;
// a server (or an instance built straight off the prototype) inherits this no-op
// and stays DOM-free.
GameSim.prototype.fx = NULL_FX;

// Defaults for the injected environment, so a prototype-built instance still has
// a clock and randomness. The constructor installs the seeded rng + chosen clock.
GameSim.prototype.clock = SystemClock;
GameSim.prototype.rng = Math.random;
